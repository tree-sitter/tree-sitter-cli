...
function grammar(baseGrammar, options) {
  ...
  if (options.externals) {
    ...
    externals = options.externals.call(externalsRuleBuilder, externalsRuleBuilder).map(normalize)
  ...
  if (options.inline) {
    ...
    const baseInlineRules = baseGrammar.inline.map(inline => inline.map(sym));
    const inlineRules = options.inline.call(ruleBuilder, ruleBuilder, baseInlineRules);

    if (!Array.isArray(inlineRules)) {
      throw new Error("Grammar's inline must be an array or rules.");
    }
  ...
  return {name, rules, extras, conflicts, externals, inline};
}
