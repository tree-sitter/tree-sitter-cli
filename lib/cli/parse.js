module.exports = function parse(options, callback) {
  ...
  const maxLength = Math.max.apply(Math, options.codePaths.map(path => path.length))
  ...
  for (const codePath of options.codePaths) {
    ...
    const duration = Math.round((t1 - t0) / options.repeat);
    const hasError = document.rootNode.toString().includes('ERROR')
